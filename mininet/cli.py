...
class CLI( Cmd ):
    ...
    def do_valid(self, line):
        args = line.split()
        if len(args) != 3:
            output('param must be 3 \n')
            return
        if args[0] not in self.mn:
            output(args[0] + ' not a host \n')
            return
        if args[1] not in self.mn:
            output(args[1] + ' not a host \n')
            return
        ...
        #         if 's' not in x[0]:
        #             call('ovs-ofctl mod-port ' + x[0] + ' ' + i.intf2.name + ' down', shell=True)
        #     if args[1] + '-' in i.intf2.name:
        #         x = i.intf1.name.split('-')
        #         if 's' in x[0]:
        #             call('ovs-ofctl mod-port ' + x[0] + ' ' + i.intf1.name + ' down', shell=True)
        #     if args[0] + '-' in i.intf1.name:
        #         x = i.intf2.name.split('-')
        #         if 's' not in x[0]:
        #             call('ovs-ofctl mod-port ' + x[0] + ' ' + i.intf2.name + ' down', shell=True)
        #     if args[0] + '-' in i.intf2.name:
        #         x = i.intf1.name.split('-')
        #         if 's' in x[0]:
        #             call('ovs-ofctl mod-port ' + x[0] + ' ' + i.intf1.name + ' down', shell=True)
